...
function GlobalPipelineUI() {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-md p-4 overflow-y-auto transition-all duration-300">
      <div className="bg-zinc-700 shadow-md border border-zinc-500 rounded-2xl p-6 sm:p-8 rounded-3xl border border-zinc-500 max-w-5xl w-full shadow-glow flex flex-col gap-6 animate-in fade-in zoom-in-95 duration-200">
        
        {/* Header bar */}
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b border-zinc-500 pb-4">
          <div>
            <span className="text-[10px] font-bold text-emerald-400 uppercase tracking-widest flex items-center gap-1.5">
              <span className={`w-2 h-2 rounded-full bg-current ${pipelineError ? 'text-danger' : activeNode === 'completed' ? 'text-success' : 'text-emerald-400 animate-ping'}`}></span>
              LangGraph Orchestration Monitor
            </span>
            <h3 className="text-lg font-extrabold text-zinc-50 mt-1 tracking-tight">{getStatusText()}</h3>
          </div>
          ...
          {(activeNode === 'completed' || pipelineError) && (
            <button
              ...
              className={`text-xs font-bold px-4 py-2 rounded-xl transition-all duration-300 active:scale-95 ${
                pipelineError 
                  ? 'bg-danger hover:bg-rose-500 text-zinc-50 shadow-glow-danger' 
                  : 'bg-success hover:bg-zinc-8000 text-zinc-50 shadow-glow-success'
              }`}
            >
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="flex-1 space-y-4">
            <h4 className="text-[11px] font-bold text-zinc-200 uppercase tracking-wider">Multi-Agent Sequence Graph</h4>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 relative">
              {NODES.map((node, idx) => {
                ...
                const borderClass = 
                  state === 'active' ? 'border-emerald-600 bg-emerald-500/10 shadow-glow' : 
                  state === 'completed' ? 'border-success/30 bg-success/5 text-zinc-200' : 
                  state === 'failed' ? 'border-danger bg-danger/10 shadow-glow-danger' : 
                  'border-zinc-500 bg-zinc-800 opacity-35';
                  ...
                  >
                    ...
                    <div className="mt-0.5 relative shrink-0">
                      {state === 'completed' ? (
                        ...
                        <div className="w-3.5 h-3.5 rounded-full border border-zinc-400"></div>
                      )}
                    </div>

                    <div className="min-w-0">
                      <h5 className={`text-xs font-bold flex items-center gap-1.5 ${
                        state === 'active' ? 'text-zinc-50' : 
                        state === 'completed' ? 'text-success' : 
                        state === 'failed' ? 'text-danger' : 'text-zinc-300'
                      }`}>
                        <span>{node.icon}</span>
                        <span>{node.label}</span>
                      </h5>
                      <p className="text-[10px] text-zinc-200 mt-1 leading-normal">
                        {node.desc}
                      </p>
                    </div>
            ...
            <div className="text-[10px] text-zinc-300 text-center pt-2 italic flex items-center justify-center gap-1.5">
              <span>💡 Proposal Agent</span>
              <span className="text-emerald-400 font-bold">⇄</span>
              <span>👁️ Supervisor Audit</span>
              <span className="text-zinc-200">(dynamic loopback occurs if confidence &lt; 7.0/10)</span>
            </div>
          </div>

          {/* Right panel: Terminal Logs Monitor Console */}
          <div className="w-full lg:w-96 flex flex-col gap-3">
            <h4 className="text-[11px] font-bold text-zinc-200 uppercase tracking-wider">Active Execution Output</h4>
            
            <div 
              ref={consoleRef}
              className="w-full h-80 lg:h-[320px] bg-slate-900 border border-zinc-500 rounded-2xl p-4 font-mono text-[10px] leading-relaxed text-emerald-400/90 overflow-y-auto flex flex-col gap-2 shadow-inner"
            >
              {pipelineLogs.map((log, idx) => {
                ...
                return (
                  <div key={idx} className={colorClass}>
                    <span className="text-zinc-300 select-none mr-1.5">&gt;</span>
                    {log}
                  </div>
                );
              })}
              ...
              {!pipelineError && activeNode !== 'completed' && (
                <div className="text-emerald-400 flex items-center gap-2 mt-1 animate-pulse">
        ...
        {pipelineError && (
          <div className="bg-danger/10 border border-danger/20 rounded-xl p-3.5 flex items-start gap-2.5">
            ...
            <div className="text-xs">
              <span className="font-bold text-zinc-50 block">Execution Terminated</span>
              <span className="text-zinc-200 leading-normal">{pipelineError}</span>
            </div>
          </div>
        )}
...
export default function App({ Component, pageProps }: AppProps) {
  return (
    <PipelineProvider>
      <div className="min-h-screen flex flex-col justify-between selection:bg-emerald-500/30 selection:text-zinc-50">
        <Component {...pageProps} />
        <GlobalPipelineUI />
      </div>
    </PipelineProvider>
